...
class CleanSharepoint():
    ...
    def fetch_site_id(self, site, site_name):
        """
        1. Import the Class function defined in helpers
        2. Headers contains the token to log in the microsoft Graph API
        3. is_read is used two times
            only fetch the unread emails;
            and will be used to mark the emails to Read after the parsing.
        4. Try to find if the email contains attachment
        5. Parse all the content and send them to AFAS API
        """
